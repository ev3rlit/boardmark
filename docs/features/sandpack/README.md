...
- sandpack 에디터 안의 코드 수정은 sandpack 자체 상태로만 관리되며, `.md` source에 자동 반영되지 않는다. source 동기화는 후속 단계로 미룬다.
...
- note source(`.md`)로의 자동 반영은 첫 버전 범위에 포함하지 않는다.
...
- 템플릿 기본값은 `react`다.
- sandpack 에디터 내부 코드 수정의 `.md` 자동 반영은 첫 버전 범위 밖이다.
