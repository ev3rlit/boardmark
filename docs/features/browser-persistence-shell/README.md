...
사용자는 browser에서도 `.md` 문서를 단순히 열어보는 것에 그치지 않고, 같은 문서를 다시 저장할 수 있어야 한다.

핵심 요구사항:

- browser에서 로컬 `.md` 파일을 열고 다시 같은 위치에 저장할 수 있어야 한다.
...
- drag-and-drop으로 `.md`를 열 수 있어야 한다.
...
- `Open File`
  - `showOpenFilePicker`로 `.md` 파일을 선택한다.
...
- browser에서 열린 `.md`를 같은 위치에 다시 저장할 수 있다.
...
- `DataTransfer.files`에서 `.md` 또는 `.md` 파일만 허용한다.
