...
4. `x`, `y`, `w`, `h` 값을 opening line patch로 반영한다.
5. patch된 source를 repository로 다시 정규화한다.
...
- note resize는 어느 handle을 잡았는지에 따라 `x`, `y`, `w`, `h`가 함께 바뀔 수 있다.
...
2. 간단한 regex/tokenizer로 attribute를 파싱한다. (`::: note #id x=120 y=80 w=320 h=220` 형태가 고정적이므로 충분하다.)
...
```md
::: note #idea x=120 y=80 w=320 h=220 color=yellow
내용
:::
```
...
resize 후 `w=420`, `h=280`이 되면:

- 기존 `w`, `h`가 있으면 값만 교체한다.
- 왼쪽/위쪽 handle resize라면 anchor를 유지하기 위해 `x`, `y`도 함께 갱신한다.
- note canonical ordering은 `id x y w h color`를 따른다.
...
type CanvasDocumentEditIntent =
  ...
  | { kind: 'create-note'; anchorNodeId?: string; x: number; y: number; width: number; height: number; markdown: string }
...
- interaction overlay는 store 내부에 별도 슬롯으로 둔다. (예: `interactionOverrides: Map<nodeId, Partial<{x, y, w, h}>>`)
