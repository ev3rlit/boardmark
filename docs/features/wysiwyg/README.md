...
- markdown source는 계속 canonical truth로 유지해야 한다.
...
- 지원 범위 내 markdown는 source patch pipeline과 round-trip 가능해야 한다.
...
I WANT  WYSIWYG 편집 결과가 계속 markdown source로 round-trip 되고
SO THAT Git diff, raw editing, repository 기반 파이프라인을 유지할 수 있다
```
...
### 5.2 Markdown Stays Canonical

- canonical truth는 계속 markdown source fragment다.
- editor state는 source를 대체하지 않고, markdown fragment와 round-trip 되는 projection이어야 한다.
- unsupported or unstable syntax는 조용히 손실되면 안 된다.
...
- 기본 편집 모델은 block editor imitation보다 일반 문서 편집기처럼 자연스러운 rich text editing에 가깝다.
- 사용자는 raw markdown punctuation 자체를 항상 직접 다루지 않아도 된다.
- 편집의 기본 단위는 semantic block / inline mark여야 한다.
