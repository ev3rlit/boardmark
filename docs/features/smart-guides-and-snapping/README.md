...
초기 구현 범위는 회전 없는 axis-aligned object를 기준으로 한다.
...
edge label, connector routing, rotated object geometry는 별도 확장 범위로 다룬다.

### 6.5 Clutter Rule

모든 가능한 guide를 다 보여주면 안 된다.

우선 노출할 guide 우선순위는 아래처럼 둔다.
...
grid snapping은 기능군의 일부로 지원한다.

- Boardmark는 자유 배치 성향이 강하므로 object alignment snapping과 충돌하지 않도록 정책이 명확해야 한다.
- grid snapping의 on/off와 grid size는 설정 가능한 contract로 분리하는 것이 좋다.
- object-to-object alignment와 grid alignment가 동시에 후보가 될 때는 composer가 우선순위를 결정해야 한다.
...
spacing guide는 equal spacing 상태를 시각적으로 보여줘야 하며, threshold 안에서는 spacing snap 후보도 만들 수 있어야 한다.
...
drag와 resize 모두 preview 단계에서 guide와 snapping이 일관되게 동작해야 한다.
...
아래와 같은 override 수단을 둘 수 있다.

- modifier key로 snap temporarily disable
- settings에서 grid snap on/off

이 동작들은 기본 snapping 경험을 해치지 않도록 예측 가능하게 설계해야 한다.
...
- grid snapping
- spacing guides 및 spacing snapping
- multi-selection bounding-box snapping
- resize guides + snapping
- snap disable modifier 및 설정 기반 제어 확장
...
핵심은 `object-alignment-snapping.ts`, `grid-snapping.ts`, `spacing-guides.ts`가 **동등한 레벨의 peer**라는 점이다.
...
```ts
type GuideModule = {
  id: 'object-alignment-snapping' | 'grid-snapping' | 'spacing-guides'
...
- spacing visual과 spacing adjustment 후보를 함께 다룰 수 있다.
- object alignment snapping이나 grid snapping의 adjustment 규칙을 재사용하지 않는다.
...
flowchart LR
  ...
  Registry --> Obj["ObjectAlignmentSnappingModule"]
...
- `ObjectAlignmentSnappingModule`은 `GridSnappingModule`을 모른다.
- `SpacingGuidesModule`은 object alignment snapping 결과를 입력으로 받지 않는다.
...
#### Step 3. `object-alignment-snapping`
...
- 독립 peer로 추가
- object alignment snapping과 별도 candidate source 유지

#### Step 5. `spacing-guides`

- equal spacing visual과 spacing adjustment 후보를 함께 연결
- object/grid snapping과 독립된 peer 규칙으로 유지
...
**Object Alignment Snapping, Grid Snapping, Spacing Guides는 하나의 복합 기능이지만, 구현 구조에서는 하나의 단위로 관리하면 안 된다. 수평 peer 모듈 + 공통 contract + 별도 composition 경계가 맞다.**
