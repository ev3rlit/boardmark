...
2. **Markdown import/export compatibility** — block-first editor state를 markdown import/export 포맷과 안정적으로 연결할 수 있어야 한다.
...
| **Markdown import/export 적합성** | 중상. 공식 Markdown extension이 있으나 현재 Beta다. custom tokenizer/serializer 확장이 가능하다. | 중하. markdown는 plugin/helper 영역이고 first-class source model은 아니다. | 중상. 직접 parser/serializer를 설계할 수 있어 fidelity 제어 폭이 가장 크다. | 높음. framework 자체가 WYSIWYG markdown editor를 지향한다. |
...
- markdown는 core model이 아니라 plugin/helper 성격이라 import/export fidelity 책임이 커진다.
...
- Lexical은 UX 잠재력은 크지만 markdown import/export 안정성 요구사항과 거리가 있다.
