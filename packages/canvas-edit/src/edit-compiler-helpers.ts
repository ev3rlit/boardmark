...
import type { CanvasObjectArrangeMode } from './edit-types'
import type {
...
export function readGroupIdFromBlock(block: string) {
  const header = parseDirectiveHeader(block.split('\n', 1)[0] ?? '')
  if (header.isErr() || header.value.name !== 'group') return null
  const id = header.value.metadata.id
...
export function readGroupZFromBlock(block: string) {
  const header = parseDirectiveHeader(block.split('\n', 1)[0] ?? '')
  if (header.isErr() || header.value.name !== 'group') return null
  const z = header.value.metadata.z
...
function orderMetadata(name: string, metadata: Record<string, unknown>) {
  ...
  return Object.fromEntries([
    ...
    ...Object.entries(metadata).filter(([key]) => !Object.hasOwn(ordered, key))
  ])
}
